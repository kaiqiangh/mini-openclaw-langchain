...
const {
  ...
  mockGetSessions: vi.fn(async (_scope?: string, agentId?: string) =>
    agentId === "elon" ? [{ session_id: "e1", title: "Elon Session", created_at: 0, updated_at: 0 }] : [],
  ),
  mockCreateSession: vi.fn(async () => ({ session_id: "s1", title: "New Session" })),
  mockArchiveSession: vi.fn(async () => undefined),
  mockRestoreSession: vi.fn(async () => undefined),
  mockDeleteSession: vi.fn(async () => undefined),
  mockGetSessionHistory: vi.fn(async () => ({ session_id: "s1", messages: [] })),
  mockReadWorkspaceFile: vi.fn(async (path: string, agentId?: string) => `content:${agentId ?? "default"}:${path}`),
...
function Probe() {
  ...
  return (
    <div>
      ...
      <button onClick={() => store.setSelectedFilePath("workspace/AGENTS.md")}>switch-file</button>
      <button onClick={() => store.updateSelectedFileContent("edited")}>edit</button>
...
function AgentProbe() {
  ...
  return (
    <div>
      ...
      <button onClick={() => store.setCurrentAgent("elon")}>switch-agent</button>
    </div>
  );
}
...
describe("store editor save flow", () => {
  it("loads file, edits content, and saves through API", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("file")).toHaveTextContent("workspace/AGENTS.md"));

    fireEvent.click(screen.getByText("edit"));
    fireEvent.click(screen.getByText("save"));

    await waitFor(() => expect(mockSaveWorkspaceFile).toHaveBeenCalled());
    expect(mockSaveWorkspaceFile).toHaveBeenLastCalledWith("workspace/AGENTS.md", "edited", "default");
  });
  ...
  it("switches session and file context by selected agent", async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("agent")).toHaveTextContent("elon"));
    await waitFor(() => expect(screen.getByTestId("session-count")).toHaveTextContent("1"));
    await waitFor(() => expect(screen.getByTestId("file-content")).toHaveTextContent("content:elon:memory/MEMORY.md"));
  });
});
