...
function appendDebugEvent(message: ChatMessage, type: string, data: unknown): ChatMessage {
  ...
  if (previous && previous.type === nextEvent.type && JSON.stringify(previous.data) === JSON.stringify(data)) {
    return message;
  }
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [sessionsScope, setSessionsScopeState] = useState<"active" | "archived">("active");
  const [sessions, setSessions] = useState<SessionMeta[]>([]);
  const [currentSessionId, setCurrentSessionId] = useState<string | null>(null);
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [isStreaming, setIsStreaming] = useState(false);
  const [selectedFilePath, setSelectedFilePathState] = useState("memory/MEMORY.md");
  ...
  const setCurrentAgent = useCallback(
    async (agentId: string) => {
      ...
      try {
        ...
        const initialContent = await readWorkspaceFile("memory/MEMORY.md", agentId);
        if (agentSwitchEpochRef.current !== switchEpoch) return;
        setSelectedFilePathState("memory/MEMORY.md");
  ...
  const deleteAgentById = useCallback(
    async (agentId: string) => {
      ...
      const fallback = next.find((item) => item.agent_id === "default")?.agent_id ?? next[0]?.agent_id ?? "default";
  ...
  const selectSession = useCallback(
    async (sessionId: string) => {
      setError(null);
      await loadSession(sessionId, sessionsScope === "archived", currentAgentId);
    },
  ...
  const saveSelectedFile = useCallback(async () => {
    setError(null);
    await saveWorkspaceFile(selectedFilePath, selectedFileContent, currentAgentId);
    setFileDirty(false);
  ...
  const setSessionsScope = useCallback(
    async (scope: "active" | "archived") => {
      ...
      if (list.length > 0) {
        await loadSession(list[0].session_id, scope === "archived", currentAgentId);
      } else {
        setCurrentSessionId(null);
        setMessages([]);
      }
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      ...
      if (sessionsScope === "archived") {
        setError("Archived sessions are read-only. Restore the session to continue chatting.");
        return;
      }
        ...
        await streamChat(
          ...
          ({ event, data }) => {
            ...
            setMessages((prev) => {
              ...
              if (event === "token") {
                const piece =
                  typeof parsed === "object" && parsed !== null
                    ...
                    : "";
                next[idx] = { ...active, content: active.content + (piece ?? "") };
                return next;
              }
              ...
              if (event === "retrieval") {
                const retrievals =
                  typeof parsed === "object" && parsed !== null
                    ...
                    : [];
                next[idx] = appendDebugEvent({ ...active, retrievals }, "retrieval", parsed);
                return next;
              }

              if (event === "tool_start") {
                const payload = (parsed as { tool?: string; input?: unknown }) ?? {};
                next[idx] = appendDebugEvent(
                  {
                    ...active,
                    toolCalls: [...active.toolCalls, { tool: payload.tool ?? "tool", input: payload.input }],
                  },
                  "tool_start",
                  payload,
                );
              ...
              if (event === "tool_end") {
                const payload = (parsed as { output?: unknown; tool?: string }) ?? {};
                ...
                next[idx] = appendDebugEvent({ ...active, toolCalls }, "tool_end", payload);
                return next;
              }

              if (event === "run_start" || event === "agent_update" || event === "reasoning" || event === "usage") {
              ...
              if (event === "done") {
                const payload = (parsed as { content?: string; run_id?: string; token_source?: string }) ?? {};
    ...
    [currentAgentId, currentSessionId, isStreaming, refreshSessions, sessionsScope],
  );
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        const selected =
          effectiveAgents.find((item) => item.agent_id === "default")?.agent_id ??
        ...
        const initialContent = await readWorkspaceFile("memory/MEMORY.md", selected);
        if (cancelled) return;
        setSelectedFilePathState("memory/MEMORY.md");
        setSelectedFileContent(initialContent);
        setFileDirty(false);
