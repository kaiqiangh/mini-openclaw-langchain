...
export function DebugTrace({ events }: Props) {
  return (
    <details className="mt-3 rounded-md border border-[var(--border)] bg-[var(--surface-3)] p-3 text-sm">
      <summary className="cursor-pointer font-medium text-[var(--text)]">Agent Debug Trace</summary>
      <div className="mt-2 space-y-2 text-xs">
        {events.map((event) => (
          <div key={event.id} className="rounded border border-[var(--border)] bg-[var(--surface-2)] p-2">
            ...
            <div className="mt-1 break-all text-[var(--muted)]">{summarize(event.data)}</div>
