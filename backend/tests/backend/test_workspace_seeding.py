...
def test_workspace_seed_copies_default_config_once(tmp_path: Path):
    ...
    (template_dir / "memory").mkdir(parents=True, exist_ok=True)
    (template_dir / "knowledge").mkdir(parents=True, exist_ok=True)
    (base_dir / "config.json").write_text('{"rag_mode": false, "agent_runtime": {"max_steps": 20}}\n', encoding="utf-8")
    ...
    (alpha_root / "config.json").write_text('{"rag_mode": true, "agent_runtime": {"max_steps": 5}}\n', encoding="utf-8")
    (base_dir / "config.json").write_text('{"rag_mode": false, "agent_runtime": {"max_steps": 99}}\n', encoding="utf-8")
