...
def test_chat_non_stream_respects_first_turn_flag(client):
    session_id = client.post("/api/v1/agents/default/sessions", json={}).json()["data"]["session_id"]
...
def test_runtime_config_endpoint_roundtrip(client):
    ...
    updated = client.put("/api/v1/agents/default/config/runtime", json={"config": payload})
...
def test_tokens_session_uses_agent_effective_runtime(client):
    ...
    alpha_session = client.post("/api/v1/agents/alpha/sessions", json={}).json()["data"][
        "session_id"
    ]
    ...
    default_tokens = client.get(f"/api/v1/agents/default/tokens/session/{default_session}")
